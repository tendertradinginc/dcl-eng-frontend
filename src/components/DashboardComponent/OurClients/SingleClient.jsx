...
const SingleClient = ({ clientData, index, setReload }) => {
  ...
  return (
    <tr className={index % 2 === 1 ? "bg-[#f2f2f2]" : ""}>
      ...
      <td className="py-1 pl-36 text-gray-700">
        <Image
          loader={customLoader}
          src={image}
          alt="client"
      ...
      <td className="px-16 py-1 pl-32 font-semibold text-si-primary">{name}</td>

      {/* <td className="py-1 pr-2 font-semibold text-si-primary">{details}</td> */}

      <td className="flex items-center space-x-3 pl-16 py-2 md:px-4">
        <AlertDialog >
          <EditClientModal
            data={clientData}
            setReload={setReload}
